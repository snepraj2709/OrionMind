...
- **Locked steamgraph:** The personal placeholder and sample preview share one smooth, accessible SVG steamgraph backed by typed local fixtures. Theme colors resolve through the canonical registry, and a screen-reader data table supplies the chart alternative.
- **Permanent preview:** The locked reference is the intentional default for every signed-in user. Its 18-of-30-day and 9-of-15-entry progress, copy, and graph data stay fixed and user-independent; the range selector only changes the deterministic sample window. The existing repository boundary remains available for a future approved longitudinal integration.
