...
Production always uses the authenticated HTTP repository when the protected Reflections screen mounts. Backend rollout and cohort controls remain fail-closed and surface through the screen's technical error and retry state. Available insights without evidence in the selected range use a tab-level no-results state; inner tensions render only evidence-backed items. Reflection fixtures, response builders, and mock repositories are test-only and are not exported through the production feature entrypoint.
